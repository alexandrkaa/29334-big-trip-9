...
export const oneRoute = () => {
  const routeAction = Array.from(routeActions).sort(() => 0.5 - Math.random()).slice(0, 1)[0];
  const routePlace = routePlaces.filter((it) => it.type === routeAction.type).sort(() => 0.5 - Math.random()).slice(0, 1)[0];
  return {
